...
float MeasureAngle(const Vec3f &a, const Vec3f &b, int a_label, int b_label) {
  if(a_label != b_label || isnan(a) || isnan(b) || find(flat_labels.begin(), flat_labels.end(), a_label) == flat_labels.end())
    return 100.0;
...
int BuildGraph(const Mat &labels, const Mat &normals, vector<Edge> *edges) {
...
void SegmentGraph(const vector<Edge> &edges, Universe *universe) {
  ...
  while(pEdge != edges.end()) {
    ...
    //cout << "A: " << a << ", B: " << b << ", W: " << pEdge->weight << endl;
...
void ConnectedComponents(const Mat &labels, Mat *normals) {
  ...
  int num_edges = BuildGraph(labels, *normals, &edges);
  ...
  while(pNormals != normals->end<Vec3f>()) {
    if(*pSegments >= 0 && find(flat_labels.begin(), flat_labels.end(), *pLabels) != flat_labels.end()) {
      //cout << *pLabels << endl;
